...
**Skill:** `rook-android-api-sources` · **Author:** ROOK
...
```bash
npx skills add RookeriesDevelopment/rook-skills --skill rook-android-api-sources
```

Target a specific agent and skip prompts:

```bash
npx skills add RookeriesDevelopment/rook-skills --skill rook-android-api-sources -a claude-code -y
```
...
# Claude / Claude Code:
cp -r rook-skills/skills/rook-android-api-sources .claude/skills/

# Codex / Antigravity / Cursor:
cp -r rook-skills/skills/rook-android-api-sources .agents/skills/
```
